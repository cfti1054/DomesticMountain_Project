...
/*
  - HandlerInterceptor인터페이스
   : 컨트롤러가 요청 전과 후에 반복적으로 실행 할 기능을 구현하기 위한 인터페이스
   : 다수의 인터셉터도 구현할 수 있다.
   : 로그인 검사, 응답시간 계산, 이벤트 기간 만료등에서 이용하면 편리 
   : 인터셉터를 적용하기 위해서 HandlerInterceptor를 등록해야 한다.
   	<mvc: Intercepter/> 태그 이용
 */
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		try {
			...
			if(info == null) {
				...
				// 로그인이 된 경우
				// 관리자 메뉴는 usership 51 이상만 가능 
				if(uri.indexOf("admin") != -1 && info.getUsership() < 51) {
					result = false;
