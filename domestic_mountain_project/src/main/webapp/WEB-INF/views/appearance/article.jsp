...
<c:if test="${sessionScope.loginUser.userid==dto.user_id||sessionScope.loginUser.usership>50}">
		...
		<div class="body-main">
						...
						<td colspan="2">
							<c:if test="${not empty dto.saveFilename}">
								<p class="border text-secondary my-1 p-2">
									...
									<a href="${pageContext.request.contextPath}/appearance/download?post_num=${dto.post_num}">${dto.originalFilename}</a>
								</p>
							</c:if>						
						</td>
					</tr>
					...
					<tr>
						<td colspan="2">
							...
							<c:if test="${not empty prevDto}">
								<a href="${pageContext.request.contextPath}/appearance/article?${query}&post_num=${prevDto.post_num}">${prevDto.post_title}</a>
							</c:if>
						</td>
					</tr>
					<tr>
						<td colspan="2">
							...
							<c:if test="${not empty nextDto}">
								<a href="${pageContext.request.contextPath}/appearance/article?${query}&post_num=${nextDto.post_num}">${nextDto.post_title}</a>
							</c:if>
						</td>
					</tr>
				</tbody>
			...
			<table class="table table-borderless mb-2">
				<tr>
					<td width="50%">
						<c:choose>
							<c:when test="${sessionScope.loginUser.userid==dto.user_id}">
								<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/appearance/update?post_num=${dto.post_num}&page=${page}';">수정</button>
							</c:when>
							<c:otherwise>
					...
					<td class="text-end">
						<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/appearance/list?${query}';">리스트</button>
					</td>
				</tr>
			</table>
