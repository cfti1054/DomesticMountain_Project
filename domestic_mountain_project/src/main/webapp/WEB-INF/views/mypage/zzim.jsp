...
.btn1.small, .btn-two.small, .btn-gradient.small, .btn-effect.small {
	padding: 8px 18px;
	font-size: 14px;
}
							...
							<div
								...
								<c:choose>
									<c:when test="${list.size() > 0}">
										<h3 style="font-weight: bold;">${list[0].user_name}님찜목록</h3>
									</c:when>
								</c:choose>
								<button type="button"
									class="delete-button btn1 btn-gradient red small zzim-deleteCheck"
									onclick="deleteZzimSelect();">선택 삭제</button>
							</div>
													...
													<div class="product-item">
														<figure>
															...
															<a href="#" class="btn-wishlist"
																onclick="#"><i class="bi bi-heart"
																style="width: 30px; height: 22px;"></i></a>
															<a
																href="${pageContext.request.contextPath}/product/${dto.product_num}">
																<img
														...
														<hr>
														<span class="price"><label><fmt:formatNumber
																	value="${dto.product_price}" />원</label></span>
														<div
															class="d-flex align-items-center justify-content-between">

															<a href="#" class="nav-link">Add to Cart <i
																class="bi bi-cart"></i></a> <input class="checkbox"
																name="nums" type="checkbox" value="${dto.product_num}"
																style="zoom: 2.0;">
														</div>
													</div>
												</div>
							...
							<c:choose>
								<c:when test="${list.size() == 0}">
									<div class="mt-3 p-3 text-center"
										style="font-weight: bold; font-size: x-large;">찜목록이
										없습니다.</div>
								</c:when>
								<c:otherwise>
